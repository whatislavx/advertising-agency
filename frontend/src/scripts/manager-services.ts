...
function init() {
    ...
    try {
        ...
        // 1. Прив'язка кнопок додавання
        const btnAddService = document.getElementById('btn-add-service');
        if (btnAddService) {
            console.log('Attaching listener to btn-add-service');
            btnAddService.addEventListener('click', (e) => {
                e.preventDefault();
                openAddServiceModal();
            });
        } else {
            console.error('btn-add-service not found');
        }

        const btnAddResource = document.getElementById('btn-add-resource');
        if (btnAddResource) {
            console.log('Attaching listener to btn-add-resource');
            btnAddResource.addEventListener('click', (e) => {
                e.preventDefault();
                openAddResourceModal();
            });
        } else {
            console.error('btn-add-resource not found');
        }
