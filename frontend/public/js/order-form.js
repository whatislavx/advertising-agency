...
function renderPage() {
    ...
    if (serviceNameDisplay)
        serviceNameDisplay.innerText = currentService.name;
    if (basePriceDisplay)
        basePriceDisplay.innerText = formatCurrency(currentService.basePrice);
    if (summaryBasePrice)
        summaryBasePrice.innerText = formatCurrency(currentService.basePrice);
    ...
    if (resourceContainer) {
        ...
        mockDB.resources.forEach(res => {
            ...
            card.addEventListener('click', function () { toggleResource(this); });
            card.innerHTML = `
